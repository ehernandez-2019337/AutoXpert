...
public class SucursalDAO {
 ...
 // SPListar   
    ...
    public int agregar(Sucursal sc){
        String sql = "Insert into Sucursal (nombreSucursal, direccion, telefonoSucursal, correoSucursal) values(?, ?, ?, ? )";
        try{
            con = cn.Conexion();
            ps = con.prepareCall(sql);
    ...
    public int  actualizar(Sucursal su){
        String sql = "Update Sucursal set nombreSucursal =?, direccion =?, telefonoSucursal =?, correoSucursal =? where codigoSucursal = ?";
        try{
            con = cn.Conexion();
            ps = con.prepareCall(sql);
            ...
            ps.setInt(5, su.getCodigoSucursal());
